...
export default function PropertiesPanel() {
  ...
  // verifica se deve mostrar o painel de configuração de criação
  const showObjectConfig = !selectedObj
    && (activeTool === "structure" || activeTool === "event");
...
// ─── PAINEL DE CONFIGURAÇÃO DE CRIAÇÃO ───────────────
function ObjectConfigPanel({ theme:t, activeTool, selectedStructureType, selectedEventType, objectConfig, onUpdate }) {
  const cfg = activeTool === "structure"
    ? STRUCTURE_TYPES[selectedStructureType]
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize:11, color:t.textMuted }}>
          Configure antes de adicionar ao canvas
        </div>
      </div>

      <SectionLabel label="Dimensões" theme={t} />

      {/* largura */}
      <Field label="Largura (metros)" theme={t}>
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          <input
            ...
            style={inputStyle(t)}
          />
          <span style={{ fontSize:11, color:t.textMuted, flexShrink:0 }}>m</span>
        </div>
      </Field>

      {/* altura */}
      <Field label="Altura (metros)" theme={t}>
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          <input
            type="number"
            min={0.01}
      ...
      <SectionLabel label="Quantidade" theme={t} />

      {/* quantidade */}
      <Field label="Criar quantos objetos de uma vez?" theme={t}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <button
            onClick={() => onUpdate({ quantity: Math.max(1, objectConfig.quantity - 1) })}
            style={{
          ...
          <input
            ...
            min={1}
            max={50}
            value={objectConfig.quantity}
            onChange={(e) => onUpdate({ quantity: Math.max(1, Math.min(50, Number(e.target.value))) })}
            style={{ ...inputStyle(t), textAlign:"center", flex:1 }}
          />
          <button
            onClick={() => onUpdate({ quantity: Math.min(50, objectConfig.quantity + 1) })}
            style={{
              width:36, height:36, borderRadius:8,
              border:`1px solid ${t.border}`, background:t.bg,
              color:t.text, fontSize:20, cursor:"pointer",
      ...
      <div style={{
        ...
        <div style={{ fontSize:12, color:t.text }}>
          📐 {objectConfig.width}m × {objectConfig.height}m
        </div>
        <div style={{ fontSize:12, color:t.text, marginTop:4 }}>
          📦 {objectConfig.quantity} {objectConfig.quantity === 1 ? "objeto" : "objetos"}
        </div>
        <div style={{ fontSize:11, color:t.textMuted, marginTop:6, lineHeight:1.5 }}>
          Clique no canvas para adicionar {objectConfig.quantity === 1 ? "o objeto" : "os objetos"} na posição desejada.
        </div>
      </div>
    </div>
  );
...
function ObjectProperties({ obj, theme:t, onUpdate, onRemove, onDuplicate }) {
  ...
  // converter células para metros
  const cellsToMeters = (cells) => cells;
  const metersToCells = (meters) => Math.max(1, Math.round(meters));
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginTop:6 }}>
              {Object.entries(SEAT_TYPES).map(([key, val]) => (
                ...
                >
                  ...
                  <span style={{ fontSize:9, color:obj.seatType===key ? val.color : t.text, fontWeight:600 }}>
                    {val.label}
                  </span>
                </button>
              ))}
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:4, marginTop:6 }}>
              {Object.entries(SEAT_STATUS).map(([key, val]) => (
                ...
                >
                  ...
                  <span style={{ fontSize:12, color:obj.status===key ? t.primary : t.text, fontWeight:obj.status===key ? 700 : 400 }}>
                    {val.label}
                  </span>
                </button>
              ))}
      ...
      {/* estrutura/evento — editar dimensões em metros */}
      {!isSeat && (
        <>
          <SectionLabel label="Dimensões" theme={t} />
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
            <Field label="Largura (m)" theme={t}>
              <input
                type="number"
                min={0.5}
                max={50}
                step={0.5}
                value={cellsToMeters(obj.w || 2)}
                onChange={(e) => onUpdate({ w: metersToCells(Number(e.target.value)) })}
                style={inputStyle(t)}
              />
            </Field>
            <Field label="Altura (m)" theme={t}>
              <input
                type="number"
                min={0.5}
                max={50}
                step={0.5}
                value={cellsToMeters(obj.h || 1)}
                onChange={(e) => onUpdate({ h: metersToCells(Number(e.target.value)) })}
                style={inputStyle(t)}
              />
            </Field>
          </div>
      ...
      <div>
        ...
        <div style={{ marginTop:8, display:"flex", flexDirection:"column", gap:8 }}>
          {[
            ...
            <div key={cfg.key} style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          {Object.entries(SEAT_TYPES).map(([key, val]) =>
            capacity[key] ? (
              <div key={key} style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                ...
                <span style={{ fontSize:12, fontWeight:700, color:t.text }}>{capacity[key]}</span>
              </div>
            ) : null
          )}
          {capacity.total > 0 && (
            <div style={{
              ...
              <span style={{ fontSize:14, fontWeight:800, color:t.primary }}>{capacity.total}</span>
            </div>
          )}
        </div>
      </div>
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      {SETORES.map((setor) => (
        <div key={setor.key} style={{
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ width:12, height:12, borderRadius:3, background:setor.color }} />
            <span style={{ fontSize:13, fontWeight:600, color:t.text }}>{setor.label}</span>
          </div>
          <span style={{
            fontSize:12, fontWeight:700, color:setor.color,
            background:`${setor.color}22`, padding:"2px 8px", borderRadius:20,
          }}>{countBySector[setor.key] || 0} lugares</span>
        </div>
      ))}
    </div>
  );
...
function CamadasPanel({ theme:t, objects, selectedId, onSelect, onRemove }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
        ...
        return (
          ...
          >
            <div style={{ display:"flex", alignItems:"center", gap:6 }}>
              ...
              <span style={{ fontSize:11, color:t.text, fontWeight:600 }}>{obj.label}</span>
            </div>
            <button
      ...
      {objects.length === 0 && (
        <div style={{ textAlign:"center", color:t.textMuted, fontSize:12, padding:"20px 0", lineHeight:1.6 }}>
...
function SectionLabel({ label, theme:t }) {
  return (
    <div style={{ fontSize:11, fontWeight:700, color:t.textMuted, textTransform:"uppercase", letterSpacing:1, marginBottom:4 }}>
      {label}
    </div>
  );
}
...
function Field({ label, children, theme:t }) {
  return (
    <div>
      <div style={{ fontSize:10, color:t.textMuted, marginBottom:4, fontWeight:600 }}>{label}</div>
      {children}
    </div>
  );
}
